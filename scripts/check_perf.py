...
BUDGETS = {
    ...
    #
    # 重排素材后重新量（total 保持 6700，没往上顶）：
    #   6610 → 6767  四处「一图三用 / 器材图鉴」换成协会自有实拍（bench-wide 147
    #                + rf-modules-board 73 + rf-amp-lcd 28 + contest-inspect 50）
    #   6767 → 6602  走廊兜底图的导出尺寸从 1600 收到 900。走廊是等高胶片条、
    #                一格 432 CSS px 宽，DPR2 只要 864 —— 1600 是轮播时代的数字，
    #                搬到胶片条之后一直没跟着改（pcb 132→52、etched 122→38）。
    #                **这一刀是纯赚的**：屏幕上看不出区别，所以上面那 157KB 不用
    #                拿预算去换。
    #   6602 → 6565  60 人的全员合影从走廊搬到分镜 02 的整宽带（16/5，1600x500，
    #                107KB），走廊那格换成 21 人的出征合影（943x707，87KB）。
    # 现在留 135KB 余量。
    #
    # **这条预算量的是「load 事件那一刻已经到手的字节」，不是「这一页总共多少图」。**
    # 浏览器把 loading="lazy" 的图排在 load 之后，所以首屏外的照片是一张张陆续到
    # 的：同一个页面同一份代码，这里量到 img 680KB，而多等一会儿（shoot.py 那条
