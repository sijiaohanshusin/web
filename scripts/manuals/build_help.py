...
def make_book(audience, title):
    ...
    doc.add_paragraph("版本说明：常规流程沿用 2026 年 9 月 4 日正式版截图；成员展示、用户名、批量晋升保护与勋章授予改进为 2026 年 9 月 5 日候选版，尚未上线。以每篇核验记录为准。")
    ...
    for number, (meta, body, slug) in enumerate(items, 1):
        task_label = doc.add_paragraph(f"任务 {number:02d}    {'系统管理员' if meta['access'] == 'admin' else '内部操作' if audience == 'admin' else '操作指南'}", "Subtitle")
        task_label.paragraph_format.page_break_before = True
        ...
        for line in body.strip().splitlines():
            ...
            if shot:
                ...
                if w < 600:
                    width = min(width, 82 if h > w * 1.1 else 130)
                p = doc.add_paragraph()
                ...
                note = re.sub(r'^图\s*\d+-\d+[\s\u3000]*', '', line.strip('*'))
                if last_caption is not None and note not in last_caption.text:
                    last_caption.add_run('。' + note)
            else:
                p = doc.add_paragraph()
                inline(p, line)
        ...
        p = doc.add_paragraph(f"核验日期 {meta['verified']}    适用版本 {meta['version']}    截图编号见网页源稿  ", "Caption")
