...
def make_book(audience, title):
    ...
    doc.add_paragraph("版本说明：常规流程沿用 2026 年 9 月 4 日正式版 9c9b9e8 截图；成员展示新流程为 2026 年 9 月 5 日本地 v4 验收版，尚未上线。最终版本以每篇的核验记录为准。")
    ...
    doc.add_heading("任务目录", 1)
    for number, (meta, _, slug) in enumerate(items, 1):
        p = doc.add_paragraph()
        p.add_run(f"{number:02d}  ").bold = True
        link(p, meta["title"], f"https://heuesta.cn/help/{audience}/{slug}/")
    ...
    for number, (meta, body, slug) in enumerate(items, 1):
        ...
        doc.add_heading(re.sub(r"[，。；：、·（）→]", " ", meta["title"]), 1)
        ...
        for line in body.strip().splitlines():
            ...
            if shot:
                ...
                caption = doc.add_paragraph(label, "Caption")
