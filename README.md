...
成员展示 v4、三类任务式帮助中心和质量门槛在候选分支中验收，尚未全部发布。网页帮助以 `/help/` 为入口，支持按任务搜索与可展开的首次使用路线；管理内容及截图由服务端按权限过滤。后续 PDF 三册聚焦注册、完善资料和角色对应的第一次操作，详细参考保留在网页。路线引用同一 Markdown 内容源，不再维护两份不同正文；旧全文导出不是新版首次引导定稿。
