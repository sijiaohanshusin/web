...
class HonorPublicationTests(TestCase):
    ...
    def test_certificate_private_draft_live_and_revoked(self):
        ...
        # Consume the test client's wrapper so cleanup preserves TestCase's DB transaction.
        private_body = b''.join(own_response.streaming_content)
        self.assertTrue(private_body.startswith(b'\xff\xd8'))
        self.publish()
        response = visitor.get(image.public_url)
        self.assertEqual(response.status_code, 200)
        self.assertIn('no-store', response['Cache-Control'])
        self.assertEqual(b''.join(response.streaming_content), private_body)
