...
class RecognitionTests(TestCase):
    ...
    @override_settings(HONOR_AI_MODEL='qwen3.7-flash')
    ...
    @override_settings(HONOR_AI_MODEL='qwen3.8-max-0902', DASHSCOPE_WORKSPACE_ID='')
    def test_max_snapshot_uses_images_json_and_bounded_non_thinking_request(self):
        from . import recognition
        from .models import RecognitionTask
        self.assertEqual(recognition.available(), '')
        task = RecognitionTask.objects.get(pk=self.start(self.upload()).json()['id'])
        _, call = self.provider_response(task, {
            'choices': [{'finish_reason': 'stop', 'message': {'content': '{"title":"Example award"}'}}]})
        payload = call.kwargs['json']
    ...
    @override_settings(HONOR_AI_MODEL='qwen3.8-max-0902')
    def test_max_quota_error_never_falls_back_to_another_model(self):
        from . import recognition
        from .models import RecognitionTask
        task_id = self.start(self.upload()).json()['id']
        with patch.object(recognition, 'call_model', side_effect=recognition.RecognitionError('quota')) as call:
            self.assertTrue(recognition.process_one())
            self.assertFalse(recognition.process_one())
        call.assert_called_once()
        self.assertEqual(call.call_args.args[0].model, 'qwen3.8-max-0902')
        self.assertEqual(RecognitionTask.objects.get(pk=task_id).error_code, 'quota')
        self.assertEqual(recognition.available(), 'quota')
