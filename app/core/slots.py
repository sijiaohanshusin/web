...
SLOTS: tuple[SlotSpec, ...] = (
    ...
    SlotSpec(
        ...
        label="出征合影",
        # 曾经是 12/5，因为那时这一格放的是 60 人的全员合影 —— 走廊是等高胶片条
        # （见 home.css 的 .nf-tile），4:3 的格子只有 432 CSS px 宽，60 张脸在里面
        # 各 7px。那张照片现在有自己的整宽带（home.legacy.group），这一格因此回到
        # 4:3，换成人数正常的队伍合影。**key 刻意没改**：改 key 等于把站务可能
        # 已经传进这一格的图静默丢掉（MediaSlot 按 key 存）。
        ratio="4 / 3",
        brief="队伍合影：出征前或赛后在背景板前拍的那种，能看出赛事名与年份最好。"
              "全员大合影请放到「全员合影」那一格（首页那条整宽带），"
              "塞进这个 4:3 的方框里人脸会太小。",
        group="首页 · 科协日常",
        # 2025 年 TI 杯全国电赛出征合影（21 人，源图本来就是 4:3）。
        # 来源与裁切见 scripts/build_photos.py。
        fallback="img/photo/group-ti-2025.webp",
        priority=2,
    ),
    SlotSpec(
    ...
    # ---------------- 首页 · 分镜 02 末尾的整宽合影带 ----------------
    # 全员合影原来是走廊里的一格。走廊是等高胶片条，一格 432 CSS px 宽 —— 60 个人
    # 挤在里面，每张脸约 7px，而这是招新页面最想让人看清的一张。给它一条整宽带
    # （section 的直接子元素，`.nf-section` 左右内边距是 0，所以天生全出血），
    # 放在「31 年技术传承」那一段的末尾：那一段讲的就是「一届届人」。
    SlotSpec(
        key="home.legacy.group",
        # label 会在没有 MediaSlot.alt 时直接当 <img alt> 用，所以它得是「照片里
        # 是什么」而不是「这一格在版面上叫什么」——「全员合影（整宽带）」这种写法
        # 对读屏软件来说后半截是噪音。版面信息写进 brief。
        label="全员合影",
        # 16/5 是从这张照片本身量出来的：源图 1929x1288，人从 x=82 铺到 1862
        # （两端没余量、只能裁上下），头顶到脚下占 370px。3.2:1 刚好留出「头顶
        # 上方一截背景板 + 脚下一截反光地面」，再宽就切到人。
        ratio="16 / 5",
        brief="全员大合影，人越多越好。这一格是整宽照片带（3.2:1，横向不会被裁），"
              "所以要横幅构图：站三排让后排也露脸，室内开顶灯避免逆光。",
        group="首页 · 科协日常",
        # 2024 年全员合影（约 60 人），拍于电工电子国家级实验教学示范中心。
        # 来源与裁切见 scripts/build_photos.py 的 group-2024。
