...
代码提交 `2409f45` 已推送功能分支。首轮 GitHub Actions `34040736171` 中，论坛集成、浏览器与插件检查通过；PostgreSQL 728 项中有 10 个错误，集中在荣誉证书测试组。

首个错误来自测试直接调用流式响应的 `close()`，触发 `request_finished` 并关闭 `TestCase` 持有的 PostgreSQL 事务连接，随后同组测试级联失败。SQLite 内存库忽略这次关闭，因此本机未暴露问题。按 Django 测试客户端的 `closing_iterator_wrapper` 契约，改为完整读取 `streaming_content`，由包装器清理文件；同时断言 JPEG 数据及公开前后的图片一致。不修改生产连接管理，不关闭信号，不跳过测试。修正后重新执行完整本地回归与远端 CI；最终状态以该提交对应运行记录为准。

远端日志的对象存储在本机下载失败，使用现有 SSH 仅转读该 CI 日志到内存。没有向服务器写文件、连接生产数据库或启动测试；临时下载地址与认证信息不入库。
