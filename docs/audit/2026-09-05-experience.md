...
| HELP-01 | P2 | 文档需从头通读，截图小，离线目录跳网页 | 45 篇网页任务、搜索、检查点、图片放大；三册各12页首次引导完成 | 按角色过滤标题/正文/图片；36页已逐页视觉检查，用户阅读反馈及生产发布待完成 |
...
| DOC-01 | P2 | 离线自动排版出现少量内容独占下一页，界面缩图需放大 | 旧全文镜像归档；首次引导版按任务分页，33幅清晰局部截图，内部目录可跳转 | 新三册36页已在120dpi下逐页检查；候选验收版，不把导出成功当生产发布 |
...
| MAIL-01 | P1 | IMAP连接中断后，后续正文下载可能返回false，原解析器把失败当空正文并标记归档完成 | 候选1.2.3拒绝未完成的正文读取，进入重试，不影响合法空正文 | 真实TLS断线失败测试及解析器失败测试；重启后两封都恢复且不重复 |
| MAIL-02 | P2 | ImapFlow认证失败携带authenticationFailed/serverResponseCode，原重试只识别首个code，导致重复尝试 | 候选1.2.3统一认证识别；本轮失败立即停止，后台提示更新凭据 | 单元红/绿及真实TLS拒绝认证，只产生1次认证请求 |
| MAIL-03 | P1 | 未知错误原文和任意code进入状态及重试信息，存在记录服务器回显敏感内容的风险 | 候选1.2.3只输出固定消息和白名单代码，不复制未知原文 | 合成隐私canary不得出现在状态、重试或抛出的错误中；不声称已发现真实凭据泄露 |
| MAIL-04 | P2 | 接收时间晚于论坛时钟时，NodeBB会把邮件建为定时/删除状态，同一发件人后续拆成新主题 | 限制论坛写入时间不晚于当前时刻，邮件正文与归档元数据仍保留原接收时间 | 真实论坛合成未来日期触发失败；补测分段楼层、原始时间保留、同发件人合并和非定时状态 |
...
## 2026-09-05 隔离邮件传输与故障恢复

- 上一批 `61a1490` 的完整CI已经通过：656项PostgreSQL Django回归、30项真实论坛检查，以及浏览器/性能/静态/插件门槛。这个结果不替代本批提交的CI。
- 新增仅监听127.0.0.1的TLS/IMAP夹具，使用临时自签名证书、随机合成密码和虚构邮件。测试客户端显式信任该证书；独立反例确认不信任时连接失败，不关闭证书验证、不改正式Gmail地址、不使用真实邮件凭据。
- 本地插件测试从28项增至41项，全部通过。新增8项传输检查，涵盖基线、多层MIME/中文头、纯HTML、按接收时间排序、重连与稳定ID去重、只读命令、失败重试、UIDVALIDITY变化、缺失邮件和断线恢复。测试协议只实现客户端实际使用的子集，不能替代真实Gmail网络可达性和投递验证。
- 先复现认证重试3次、未知错误回显、断线后空正文归档三类问题，再修复。连接中断时不发布不完整正文，重试队列跨同步器重启恢复；正常下载的空邮件仍能展示占位说明。不删除或自动重写已有论坛邮件。
- `scripts/forum_fixture/library.js` 从直接调用归档改为真实TLS连接、IMAP解析、同步器和真实NodeBB/PostgreSQL持久化，再沿用六角色浏览器权限验证。增加新UID重复投递的去重检查，附件内容不传输。最终真实论坛结果以新提交CI为准，当前待运行。
- 正式站及论坛未部署、未重启，未操作真实会员或邮件。剩余计划仍包括其它后台核心写入链、备份/回退演练、用户手册反馈和分阶段发布；正式论坛更新需独立确认。
- `9bc03ca` 的PostgreSQL、全站浏览器和41项插件检查通过，真实论坛联调因未来接收时间而失败。保留这一失败案例，不通过改成过去日期来绕开；新增本地红/绿回归后修复发帖时间边界，插件增至43项全部通过。随后提交须重跑完整CI。
- 时间边界依据固定版本的 [NodeBB 4.14.0主题创建实现](https://github.com/NodeBB/NodeBB/blob/607f737bdcb0d894f09829870ff006f2f325b226/src/topics/create.js)：未来时间进入定时路径。该现象由隔离夹具复现，不声称线上已出现同样故障。
