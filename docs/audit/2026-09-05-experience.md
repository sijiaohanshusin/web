...
本轮是分阶段改进，不是全站重做。候选版本已经实现成员展示 v4、任务式帮助中心、用户名扩展和批量晋升保护；**尚未部署到正式站，整个计划未完成**。
...
| --- | --- | --- | --- |
| Q01 | 主站后端 | PostgreSQL CI 625 项测试全部通过，无跳过；包含并发保存与停用竞争 | 不等于压力测试、渗透测试或全部生产配置已验 |
| Q02 | 成员展示与帮助 | 49 组浏览器检查；五档宽度 1440、1024、768、390、320；卡片单独发布、草稿隔离、撤回、保存失败与恢复 | 还没有真实成员可用性访谈 |
| Q03 | 跨角色 | 35 个路径 × 9 种身份，共 315 组 HTTP 权限核对；真实密码登录；24 张桌面/手机截图 | 列表能打开不代表每一个后台写操作都已浏览器点验 |
...
最终质量基线记录：提交 `7313d8c` 的 [GitHub Actions run 33944421197](https://github.com/sijiaohanshusin/web/actions/runs/33944421197) 两项作业全部成功。PostgreSQL 625 项、迁移、生产静态管线、浏览器、性能预算与插件测试均执行通过。后续提交仍须重跑，不能沿用旧提交的绿灯代替检查。
...
| 通知与个人积分 | 通知 R+，积分 P+ | 所有权、已读、积分流水 | 路径矩阵及后端 | 跨流程通知到达的浏览器串联 |
...
| 职位、任期、勋章 | A | 候选搜索、任命、历史、卸任 | D/S 截图、既有专项回归 | 手机选择长姓名与多结果 |
...
| 管理列表与表单 | 3 | 3 | 2 | 待验 | 待验 | 4 | 3 | 3 | 表格不溢出整页，但侧向滚动缺提示 |
...
| QA-01 | P1 | 无持续集成，SQLite 会跳过行锁竞争；测试从错误目录运行可能发现0项 | PostgreSQL CI、正确 app 工作目录、浏览器及插件门槛 | 625 项实际执行，CI整体全绿后才进入发布 |
| QA-02 | P2 | 测试在跨页动画未结束时点验，误报消息被覆盖 | 等待真实 View Transition finished；保留命中断言 | `check_a11y`，不以删除断言消除失败 |
| ADMIN-01 | P2 | 手机后台表格只露出前几列，看不出可横向滚动；全局页面宽度检查却会通过 | 已记录，待独立优化 | 提供可见滚动提示、固定关键识别列，320/390键盘触屏可查全字段 |
| DOC-01 | P2 | 离线自动排版出现少量内容独占下一页，界面缩图需放大 | 正在调整行距、图片区域与分页 | 每页100%检查，无孤立标题、大片意外空白和不可辨认截图 |
...
这里仍有不足：表格右侧列需要滑动才能看到，页面却没有足够明显的提示。应作为下一批 P2，不把“无整页溢出”当作体验已优秀。
...
1. 手机后台列表加入横向提示或响应式记录卡，批量操作增加对象摘要、数量确认和结果对照。
...
| --- | --- | --- | --- |
| 质量基线及 P1 | 已实现 | `7313d8c` 完整CI通过 | 未发布；备份/回退未演练 |
| 成员展示 v4 | 已实现 | 浏览器核心流程已通过，真实成员测试待做 | 未发布 |
| 三类网页帮助 | 已实现 | 权限/多设备通过；离线定稿未完成 | 未发布 |
| 其余 P2 | 部分完成 | 手机后台/外部论坛/弱网仍有缺口 | 未发布 |
