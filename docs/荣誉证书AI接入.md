...
| 方法 | 路由 | 用途 |
| --- | --- | --- |
| POST | `/achievements/honors/<uuid>/images/upload/` | 单张静态安全图片，携带版本；不修改已保存草稿或公开快照 |
...
- 新荣誉编辑器默认开启“展示证书”，普通上传和 AI 上传都会选用图片供后续发布，但不会自动保存公开内容。`show_certificate` 选择保存在草稿 JSON 中，关闭时清空公开图片引用；旧草稿未选择证书时仍保持关闭。关闭已有公开证书须重新预览并发布，或撤回整条荣誉。
