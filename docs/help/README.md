...
| --- | --- | --- | --- |
| 招新成员 | `/help/recruit/` | `recruit-first-use-achievements-release` | 16 / 13 |
| 老会员 | `/help/member/` | `member-first-use-achievements-release` | 27 / 24 |
| 站务人员 | `/help/admin/` | `admin-first-use-achievements-release` | 17 / 14 |
...
**发布边界：**2026-09-06 已将成果体系版本 `ede30a6` 部署到正式站，并完成源站与公网只读验收。三册按此网站版本重新导出，移除“待上线”标记；新流程演示图仍来自隔离账号，不把导出日期当作截图日期。发布状态由 `first_use.json` 统一维护，部署记录见 `docs/audit/2026-09-06-achievements-rollout.md`。
...
`docs/manuals/`、不带 `-first-use` 的全文导出，以及 `-selfservice-preview`、`-works-wall-preview`、不带 `-release` 的成果体系候选册均归档，不再作为最新说明发放。正式册使用独立的 `-achievements-release` 文件名，不覆盖读者正在打开的旧 PDF。不带后缀的旧首次手册仅用于对照旧正式版本。保留旧文件不等于它仍然适用。
...
- `scripts/manuals/first_use.json` 维护编排、索引、步骤引用和图源，`output_suffix=achievements-release` 控制当前产物命名。
...
4. 运行 `scripts/manuals/render_help_word.py <render_docx.py 路径> recruit-first-use-achievements-release member-first-use-achievements-release admin-first-use-achievements-release --revision 2026-09-06`。当前 Windows 转换后端为 Word，渲染器生成逐页 PNG；只操作本轮文档。
